...
static void tryWriteChgrpJournal(const std::string& disk,
                                 int32_t partStart,
                                 const Superblock& sb,
    ...
    std::string content = userName + ":" + newGroup;
...
bool chgrp(const std::string& user, const std::string& grp, std::string& outMsg) {
    ...
    if (cmd::sessionUid() != 1) {
        outMsg = "Error: chgrp solo puede ser ejecutado por el usuario root.";
        return false;
    }

    if (user.empty() || grp.empty()) {
        outMsg = "Error: chgrp requiere -user y -grp.";
        return false;
    }
    ...
    // 1) validar grupo existente y activo
    bool grpOk = false;
    {
        ...
        while (std::getline(iss, line)) {
            ...
            // group line: id,G,group
            if (cols.size() >= 3) {
                int idNum = 0;
                try { idNum = std::stoi(cols[0]); } catch (...) { idNum = 0; }
                std::string tipo = cols[1];
                if (idNum > 0 && (tipo == "G" || tipo == "g")) {
                    if (cols[2] == grp) { grpOk = true; break; }
                }
            }
        }
    }
    if (!grpOk) {
        outMsg = "Error: el grupo '" + grp + "' no existe o está eliminado.";
        return false;
    }

    // 2) reescribir user cambiando su grupo
    std::istringstream iss(content);
    std::string line;
    ...
    // JOURNAL (EXT3)
    tryWriteChgrpJournal(disk, partStart, sb, user, grp);

    outMsg = "Grupo actualizado correctamente: " + user + " -> " + grp;
    return true;
}
